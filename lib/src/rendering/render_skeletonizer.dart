...
mixin _RenderSkeletonBase<R extends RenderObject> on RenderObjectWithChildMixin<R> {
  ...
  void paint(PaintingContext context, Offset offset) {
    /// If the context is a [SkeletonizerPaintingContext] we are already shading

    SkeletonizerPaintingContext? parentContext;
    if (context is SkeletonizerPaintingContext) {
      parentContext = context;
    }
    final estimatedBounds = paintBounds.shift(offset);
    final Paint shaderPaint;
    if (parentContext == null || parentContext.config.effect != config.effect) {
      shaderPaint = config.effect.createPaint(
        parentContext?.animationValue ?? animationValue,
        parentContext?.estimatedBounds ?? estimatedBounds,
        textDirection,
      );
    } else {
      shaderPaint = parentContext.shaderPaint;
    }
    final skeletonizerContext = SkeletonizerPaintingContext(
      layer: parentContext?.layer ?? layer!,
      estimatedBounds: estimatedBounds,
      shaderPaint: shaderPaint,
      config: config,
      manual: manual,
      animationValue: parentContext?.animationValue ?? animationValue,
    );
